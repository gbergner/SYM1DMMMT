...
SUBROUTINE Smallest_eigenvalue(temperature,&
     &xmat,alpha,GAMMA10d,neig,smallest_eig,myrank,nbc,&
  ...
  !***** input *****
  integer neig,nbc,info,max_iteration,nbmn,nremez,myrank
  ...
  !******************
  double complex phi1(1:nmat_block,1:nmat_block,&
       &1:nspin,-(nmargin-1):nsite_local+nmargin)
  !use multimass CG solver to obtain phi2, with nremez=1
  double complex phi2(1:1,1:nmat_block,1:nmat_block,&
       &1:nspin,-(nmargin-1):nsite_local+nmargin)
  ...
  do ieig=1,neig
     ...
     &xmat,alpha,phi1,phi2,GAMMA10d,max_err,max_iteration,iteration,myrank,nbmn,flux,info)
     do imat=1,nmat_block     
        do jmat=1,nmat_block
           do ispin=1,nspin
