...
program BFSS_DEVICE
    ...
    double precision Tmin,Tstep,Tmax
    integer filecounter,back
    ...
    read(unit_input_para,*) Tmin
    read(unit_input_para,*) Tstep
    read(unit_input_para,*) Tmax
    ...
    do while(((back.eq.0).and.(temperature.LE.Tmax)).or.((back.eq.1).and.(temperature.GE.Tmin)))
        print *, "Running at temperature=",temperature
        itraj=0
        nacceptance=0
        ntrial=0
        ncv=0
        ...
        imeasure=0
        fluctuation=0d0
        close(unit=unit_measurement)
        write(data_output_n,'(I5.5)') filecounter
        data_output_n=trim(data_output)//trim(data_output_n)//'.dat'
        filecounter=filecounter+1
        open(unit=unit_measurement,status='REPLACE',file=data_output_n,&
